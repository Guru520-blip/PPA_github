...
export default function SuppliersPage() {
  ...
  }, [search, regionFilter, typeFilter, maxPrice]);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">BTM gas, surplus hydro, curtailment zones — stranded capacity seeking offtakers</p>
        </div>
      ...
      <p className="text-sm text-gray-400">{filtered.length} of {suppliers.length} suppliers</p>
            ...
            <tbody>
              {filtered.map((s, i) => (
                <tr key={s.id} className={`border-t border-gray-800 hover:bg-gray-900 transition-colors ${i % 2 === 0 ? "bg-gray-950" : "bg-gray-900/50"}`}>
                  ...
                  <td className="px-4 py-3 text-gray-400">{s.region}</td>
